@EIP4_FAS_Mobile_Automation  @EIP4_BRB
Feature: EIP40 Bank Receipt Booking Mobile Application 
  
  UserStory: EIP40 Mobile Application Finance
 
  Description : As a EIP User i should be able to EIP40 Mobile App
@EIP4_BRB
	Scenario: As a EIP User i should be able to Login Approve Bank Receipt Booking (BRB) from EIP40 Mobile App
	
    Given Verify user can be able to Login EIPMobile Application
    When Verify user can be able to click on Work Bench
    And Verify user can de able to select BRB Document
  ...
  Scenario: As a EIP User i should be able to Login Reject Bank Receipt Booking (BRB) from EIP40 Mobile App
  ...
  Scenario: As a EIP User i should be able to Login Document View Bank Receipt Booking (BRB) from EIP40 Mobile App
  ...
  Scenario: As a EIP User i should be able to Login Document Attachment View Bank Receipt Booking (BRB) from EIP40 Mobile App
  ...
  Scenario: As a EIP User i should be able to Login Work Flow Trace Bank Receipt Booking (BRB) from EIP40 Mobile App
